...
LOG_FILE = DATA_DIR / "fix_relationship_constraints.log"
...
# 源目录和目标目录
SOURCE_DIR = DATA_DIR / "results"
TARGET_DIR = DATA_DIR / "result_fixed"
...
def fix_relationship_constraints(json_data):
    """
    修正关系约束 —— 使用 graph_model 的 snake_case 字段名
    ...
    for entity in entities:
        ...
        # 修正拼写错误: attcker -> attacker
        if entity.get("entity_type") == "attcker":
            entity["entity_type"] = "attacker"

        # 修正拼写错误: vctim -> victim
        if entity.get("entity_type") == "vctim":
    ...
    for relationship in relationships:
        rel_type_original = relationship.get("relationship_type", "")
        ...
        if not is_valid:
            ...
            if correct_rel_type and correct_rel_type != rel_type:
                ...
                relationship["relationship_type"] = correct_rel_type
                fixes_count += 1
